...
export function bumpString(versionString: string, bumpType: bump_type): string {
	...
	let [ major, minor, patch ] = [parseInt(majorStr), parseInt(minorStr), parseInt(patchStr)];
	if (isNaN(major) || isNaN(minor) || isNaN(patch)) {
		throw new Error(`Invalid verison numbers to bump. major: ${major}, minor: ${minor}, patch: ${patch}`)
	}
