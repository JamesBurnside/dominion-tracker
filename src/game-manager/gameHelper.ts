...
export const addCardStackToPlayer = (card: DominionCard, amount: number, shortName: DominionPlayerShortName, players: DominionPlayer[]): void => {
...
export const removeCardStackFromPlayer = (card: DominionCard, amount: number, shortName: DominionPlayerShortName, players: DominionPlayer[]): void => {
	const player = players.find((player) => player.shortName === shortName)
	const newAmount = (player.deck.get(card) ?? 0) - amount;
...
export const addFullPlayerNamesToPlayers = (fullPlayerName: string, players: DominionPlayer[]): void => {
...
export const addShortPlayerNamesToPlayers = (shortPlayerName: string, players: DominionPlayer[]): void => {
	// Check if it already exists
	for (const player of players) {
