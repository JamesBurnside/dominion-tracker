...
export const uploadHandler = async (): Promise<UploadResult> => {
  ...
  try {
    isEndOfGame = !!JSON.parse(
      await messageContentScript('addScoresToPlayers')
    );
  } catch (e) {
    logger.error(e);
    return {
  ...
  try {
    await axios.put(
      `https://dominion-json-bin-default-rtdb.firebaseio.com/scores/${gameNumber}.json`,
      players
    );
