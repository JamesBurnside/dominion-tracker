...
export function extractSubjectsFromLogLine(logLine: string): CardStack {
  ...
  if (cardList.length > 2) {
    ...
    logger.error(
      `Subject has multiple "ands", subjectline: ${subjectLine}`,
      true
    );
...
const knownBoonOrHexPrefixes = new Map([
  ['The', 1],
  ['Bad', 1],
  ['Delusion', 1],
  ['Envy', 1],
...
export const extractIndividualSubject = (
  card: string,
  logLine: string
): DominionSubject => {
  ...
  card = knownBoonOrHexPrefixes.has(qualifier)
    ? card
    : card.substring(card.indexOf(' ') + 1);
  ...
  if (!card || card.length < 1) {
    logger.error(
      `Card not parsed as subject succesfully, logline: ${logLine}`,
      true
    );
    return unsupportedCard;
  }
...
export const parseQualifierToInt = (qualifier: string): number => {
  ...
  if (
    qualifier === 'a' ||
    qualifier === 'an' ||
    knownBoonOrHexPrefixes.has(qualifier)
  )
    return 1;
...
export const cardDepluralizer = (cardToSearchFor: string): string => {
  ...
  if (cardPluralsDictionary.has(cardToSearchFor))
    return cardPluralsDictionary.get(cardToSearchFor);
  ...
  if (cardToSearchFor.substring(cardToSearchFor.length - 1) === 's') {
    const cardWithoutS = cardToSearchFor.substring(
      0,
      cardToSearchFor.length - 1
    );
...
export const isCardNameOfficial = (cardToSearchFor: string): boolean =>
  cardDictionary.has(cardToSearchFor);
