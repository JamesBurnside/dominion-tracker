...
describe('Action enum conversion tests', () => {
  test('stringToAction should provide known action if string is recognized', () => {
    expect(extractActionFromLogLine('A buys and gains a turtle')).toEqual(
      DominionAction.Buys_And_Gains
    );
    expect(extractActionFromLogLine('A gains a turtle')).toEqual(
      DominionAction.Gains
    );
