...
describe('Subject helper tests', () => {
  test('extractSubjectFromLogLine should return expected subjects', () => {
    ...
    expect(extractSubjectsFromLogLine('L buys and gains ')).toEqual(
      unsupportedStack
    );
    expect(extractSubjectsFromLogLine('L gains 2 ESSTATIES')).toEqual(
      unsupportedStack
    );
    ...
    expect(
      extractSubjectsFromLogLine('L buys and gains a Silver from trash.')
    ).toEqual([
      {
        type: DominionSubjectType.Card,
        card: 'Silver',
    ...
    expect(
      extractSubjectsFromLogLine('L returns a Horse to the Horse pile.')
    ).toEqual([
      {
        type: DominionSubjectType.Card,
        card: 'Horse',
    ...
    expect(
      extractSubjectsFromLogLine('C gains a Necromancer onto their drawpile.')
    ).toEqual([
      {
        type: DominionSubjectType.Card,
        card: 'Necromancer',
  ...
  test('extractIndividualSubject handles individual subjects', () => {
    // error
    expect(
      extractIndividualSubject('a ', 'a garbage logline for testing')
    ).toEqual(unsupportedCard);
    // success
    expect(
      extractIndividualSubject('a Duchy', 'another garbage logline for testing')
    ).toEqual({
      type: DominionSubjectType.Card,
      card: 'Duchy',
      amount: 1,
    ...
    expect(
      extractIndividualSubject(
        'a Death Cart',
        'another articulately written logline for testing'
      )
    ).toEqual({
      type: DominionSubjectType.Card,
      card: 'Death Cart',
  ...
  test('extractSubjectsFromLogLine should return lists of subjects', () => {
    // error
    expect(
      extractSubjectsFromLogLine('Jam gains a Duchy, 2 Estates and a Province')
    ).toEqual([
      {
        type: DominionSubjectType.Card,
        card: 'Duchy',
  ...
  test("extractSubjectsFromLogLine doesn't handle multiple 'ands'", () => {
    ...
    expect(
      extractSubjectsFromLogLine(
        'Jak gains a Duchy and 2 Estates and a Province'
      )
    ).toEqual(unsupportedStack);
  });
